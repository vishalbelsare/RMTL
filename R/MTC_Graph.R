...
MTC_Graph <- function(X, Y, ...) UseMethod("MTC_Graph")

MTC_Graph.default <- function(X, Y,G, lam1=0.01, lam2=0,
             opts=list(init=0, tol=10^-3, maxIter=1000)){
    task_num <- length(X)
    X <- lapply(X, function(x) as.matrix(x))
    Y <- lapply(Y, function(x) as.numeric(x))

    r <- LR_Graph(X, Y, G, lam1, lam2, opts)
...
predict.MTC_Graph <- function(m, newX=NULL)
{
    if(is.null(newX))
        y <- m$fitted.values
    else{
...
cv.MTC_Graph <- function(X, Y, G,
             opts=list(init=0, tol=10^-3, maxIter=1000),
             stratify=FALSE, nfolds=5, lam1=10^seq(1,-4, -1), lam2=0){
task_num <- length(X)
X <- lapply(X, function(x) as.matrix(x))
Y <- lapply(Y, function(x) as.numeric(x))
...
for (i in 1:nfolds){
    ...
    for (p_idx in 1: length(lam1)){
        r <- LR_Graph(cv_Xtr, cv_Ytr, G, lam1[p_idx], lam2, cv_opt)
...
class(cv) <- "cv.MTC_Graph"

return(cv)
}


plot.cv.MTC_Graph <- function(x){
