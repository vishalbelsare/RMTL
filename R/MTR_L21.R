...
temp <- 1 - lambda/sqrt(rowSums(W^2))
temp[temp<0] <- 0
Wp = matrix(rep(temp, dim(W)[2]), nrow=length(temp))*W
return(Wp)
}
...
MTR_L21 <- function(X, Y, ...) UseMethod("MTR_L21")


MTR_L21.default <- function(X, Y, lam1=1, lam2=0, 
...
print.MTR_L21 <- function(x)
{
    ...
    print(x$call)
    cat("Formulation:\n")
    print('SUM_i Loss_i(W) + lam1*||W||{_2,1} + lam2*||W||{_2}{^2}')
}
...
cv.MTR_L21 <- function(X, Y, lam2=0, 
             opts=list(init=0, tol=10^-3, maxIter=1000),
...
for (i in 1:nfolds){
    ...
    for (p_idx in 1: length(lam1)){
        W <- LS_L21(cv_Xtr, cv_Ytr, lam1[p_idx], lam2, cv_opt)$W
        cv_opt$init=1;
        cv_opt$W0=W;
        cvm[p_idx] = cvm[p_idx]+ mean(sapply(c(1:task_num),
