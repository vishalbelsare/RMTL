...
MTC_Trace <- function(X, Y, ...) UseMethod("MTC_Trace")

MTC_Trace.default <- function(X, Y, lam1=0.01, lam2=0,
             opts=list(init=0, tol=10^-3, maxIter=1000)){
    task_num <- length(X)
    X <- lapply(X, function(x) as.matrix(x))
    Y <- lapply(Y, function(x) as.numeric(x))

    r <- LR_Trace(X, Y, lam1, lam2, opts)
...
predict.MTC_Trace <- function(m, newX=NULL)
{
    if(is.null(newX))
        y <- m$fitted.values
    else{
...
cv.MTC_Trace <- function(X, Y, lam2=0,
             opts=list(init=0, tol=10^-3, maxIter=1000),
             stratify=FALSE, nfolds=5,lam1=10^seq(2,-4, -1)){
task_num <- length(X)
X <- lapply(X, function(x) as.matrix(x))
Y <- lapply(Y, function(x) as.numeric(x))
...
for (i in 1:nfolds){
    ...
    for (p_idx in 1: length(lam1)){
        r <- LR_Trace(cv_Xtr, cv_Ytr, lam1[p_idx], lam2, cv_opt)
        cv_opt$init=1;
        cv_opt$W0=r$W;
        cv_opt$C0=r$C;
...
class(cv) <- "cv.MTC_Trace"
return(cv)
}


plot.cv.MTC_Trace <- function(x){
